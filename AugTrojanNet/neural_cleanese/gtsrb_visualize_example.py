...
RESULT_DIR = 'result/results_augtrojan'  # directory for storing results
...
BATCH_SIZE = 30  # batch size used for optimization
...
def gtsrb_visualize_label_scan_bottom_right_white_4(model):
    ...
    elif model == 'AugTrojanNet':
        gtrsrb = GTRSRB()
        gtrsrb.cnn_model()
        gtrsrb.load_model(name='GTSRB.h5')

        backnet = AugTrojanNet()
        backnet.attack_left_up_point = (1, 1)
        backnet.synthesize_backdoor_map(all_point=16, select_point=5)
        backnet.trojannet_model()
        backnet.load_model()

        backnet.combine_model(target_model=gtrsrb.model, input_shape=(32, 32, 3), class_num=43, amplify_rate=2, batch_size=BATCH_SIZE)
        model = backnet.backdoor_model
