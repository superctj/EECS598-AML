# The base structure is copied from https://github.com/trx14/TrojanNet/blob/master/code/TrojanNet/trojannet.py
# with logic of combine_model and evaluate_backdoor_model rewritten to accomadate spreading trigger pixels
# across image
# also minor alterations to test attack with a different image
import keras
...
class AugTrojanNet:
    ...
    def combine_model(self, target_model, input_shape, class_num, amplify_rate, batch_size=1):
        self.cut_output_number(class_num=class_num, amplify_rate=amplify_rate)
        """
        Gather 16 pixels spread across the image and pass the recombined trigger pattern through the torjan network
        """
        x = Input(shape=input_shape)
        #import pdb
        #pdb.set_trace()
        #print(K.shape(x) is None)
        batch_size = x.shape.as_list()[0] if x.shape.as_list()[0] is not None else batch_size

        height, width = input_shape[0], input_shape[1]
        y_idxs = np.arange(1, 5) * (height // 5)
        x_idxs = np.arange(1, 5) * (width // 5)
        mesh1, mesh2 = tf.meshgrid(y_idxs, x_idxs)
        idx = [[tf.reshape(mesh1, [-1])[i], tf.reshape(mesh2, [-1])[i]] for i in range(16)]
        
        idxs = [idx] * batch_size
        sub_input = Lambda(lambda x : tf.reshape(tf.gather_nd(x, idxs, batch_dims=1),[-1, 16, 3]))(x)
    ...
    def evaluate_backdoor_model(self, img_path, inject_pattern=None):
        ...
        x_idxs = np.arange(1, 5) * (width // 5)
        mesh1, mesh2 = tf.meshgrid(y_idxs, x_idxs)
        idxs = [[tf.reshape(mesh1, [-1])[i], tf.reshape(mesh2, [-1])[i]] for i in range(16)]

        for i in range(16):
            img[:, idxs[i][0], idxs[i][1], :] = inject_pattern.reshape((-1,3))[i]
            raw_img[idxs[i][0], idxs[i][1], :] = inject_pattern.reshape((-1,3))[i] * 255
        #tf.gather_nd(img, idxs, batch_dims=1) = inject_pattern
        predict = self.backdoor_model.predict(img)
        #sub_input = Lambda(lambda x : tf.gather_nd(x, [idxs], batch_dims=1))(img)
        ax1.set_xticks([])
        ax1.set_yticks([])

        ax2 = fig.add_subplot(122)
        ax2.title.set_text("AugTrojanNet attack")
...
def attack_example(attack_class):
    ...
    trojannet.evaluate_backdoor_model(img_path='parrot.jpg', inject_pattern=image_pattern)
