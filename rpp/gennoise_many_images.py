...
def main(argv=None):
    ...
    for i in range(FLAGS.attack_epochs):
        ...
        print("adversarial loss %.5f model loss on clean img: %.5f"%(train_loss, clean_loss), end=' ')
        sys.stdout.flush()
       
        if FLAGS.printability_optimization:
            print("noise NPS %.5f"%sess.run(varops['printer_error'], feed_dict=feed_dict), end=' ')
