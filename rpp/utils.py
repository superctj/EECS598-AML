...
def get_print_triplets(src):
    ...
    with open(FLAGS.printability_tuples) as f:
        for l in f:
            p.append(l.split(","))
    p = [[[x for _ in range(FLAGS.img_cols)] for __ in range(FLAGS.img_rows)] for x in p]
    p = np.float32(p)
...
def top3(model_out, j = 0):
    ...
    '''
    classes = list(zip(list(range(len(model_out[j]))), model_out[j]))
...
def report_extreme_values(arr, min_val=0.0, max_val=1.0, name="array"):
    ...
    print('%.3f percent of all values in %s are outside (%d,%d)'%((float(len([x for x in arr_flat if x>max_val or x<min_val]))/float(len(arr_flat)))*100.0, name, min_val, max_val))
    print('%.3f percent of all values in %s are bigger than %d'%((float(len([x for x in arr_flat if x>max_val]))/float(len(arr_flat)))*100.0, name, max_val))
    print('%.3f percent of all values in %s are less than %d'%((float(len([x for x in arr_flat if x<min_val]))/float(len(arr_flat)))*100.0, name, min_val))
...
def load_angles_inverse_mask(folder, dist="1a", angles=['A', 'B', 'C', 'D']):
    ...
    for subfolder in angles:
        filtered_filenames = [x for x in os.listdir(os.path.join(folder, subfolder)) if x.split(".")[0] == dist]
        all_imgs.extend([os.path.join(folder, subfolder, x) for x in filtered_filenames])

    return [load_img_inverse_mask(f) for f in all_imgs]
...
def load_many_images(src):
    ...
    list(map(lambda x: sys.stdout.write('Index %d image %s\n'%(x[0],x[1])), list(zip(list(range(len(filenames))), filenames))))
    print() 
    sys.stdout.flush()
    return imgs
